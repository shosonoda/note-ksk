...
\chapter{Leanの型理論と「集合論」}

\section{型理論}
Leanは基礎付けは型理論であって，集合論ではない．
...
\begin{itemize}
    \item まず，型理論において包含関係（$\in$）は基本的な関係ではない．その代わりに，型判断（typying judgement）$:$がある．
...
\end{itemize}
  ...
  \begin{itemize}
    \item もう一つ，型理論では$\mapsto$を使わない．その代わりに，$\lambda$記法（lambda-notation）を用いる．
...
% see the dedicated
\url{https://leanprover-community.github.io/lean-perfectoid-spaces/type_theory.html} を見よ．
