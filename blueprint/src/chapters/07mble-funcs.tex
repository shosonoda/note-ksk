...
$(X,\calM)$
%\]
%で，$\calM$ が $X$ 上の $\sigma$-加法族（\cref{def:sigma-algebra}）であるもの
を可測空間という．
\end{definition}
...
以下では，写像$f : X \to Y$ による集合族 $\calE\subset\calP(Y)$ の逆像を
\[
...
\begin{proposition}[$\sigma$-加法族の誘導] \label{prop:inverse-image-sigma-algebra}
任意の写像 $f: X \to Y$ と $Y$上の$\sigma$-加法族 $\calN$ に対して，逆像 $\calM := f^{-1}(\calN)$ は$X$上の$\sigma$-加法族である．
\end{proposition}
\begin{proof}
    まず $f^{-1}(\emptyset) = \emptyset$ より $\emptyset \in \calM$ である．
    次に 
    \cref{prop:inverse-image-basic}より
...
\begin{lemma}[逆像と生成 $\sigma$-加法族の可換性] \label{lem:commute-induce-generate-sigma-algebra}
...
\begin{remark}
    ...
    可測集合の全体$\calB$はしばしば超越的なので，その素性はよく分からないが，生成系$\calE$の方はある程度分かりやすい構造を持っていることがある．例えば，開集合系$\calO$が生成するBorel集合系$\calB(\calO)$などが典型的である．
    そのような場合にこの補題は威力を発揮する．
\end{remark}
\begin{proof}
$\sigma(\calE)$ は $Y$ 上の $\sigma$-加法族である．
したがって\cref{prop:inverse-image-sigma-algebra}より
...
\]
とおく．
%再び\cref{prop:inverse-image-sigma-algebra}より，
...
さらに\cref{lem:commute-induce-generate-sigma-algebra}より，生成族について
%\[
...
\begin{definition}[可測関数空間の記号]\label{def:measurable-function-space-notation}
...
%\]
である．
一方，\cref{lem:commute-induce-generate-sigma-algebra,lem:borel-eRR}より
\[
f^{-1}(\calB(\eRR))
=f^{-1}(\sigma(\mathcal G))
=\sigma(f^{-1}(\mathcal G))
...
\cref{lem:commute-induce-generate-sigma-algebra}より
\[
f^{-1}(\calB(\RR))
=f^{-1}(\sigma(\calO(\RR)))
=\sigma(f^{-1}(\calO(\RR)))
...
%\]
とおく．
%逆像の基本性質（\cref{prop:inverse-image-basic}）より 
...
\begin{theorem}[可測写像の合成]\label{thm:measurable-map-composition}
...
\begin{corollary}[連続写像との合成]\label{thm:measurable-continuous-composition}
$(X,\calM)$ を可測空間とする．
...
\end{corollary}
\begin{proof}
$\Phi$ は連続だから，\cref{ex:continuous-function-measurable}より
$\Phi:(\RR^n,\calB(\RR^n))\to(\RR,\calB(\RR))$ は可測写像である．
したがって\cref{thm:measurable-map-composition}より $\Phi\circ F$ は可測である．
\end{proof}
...
\begin{definition}[各点の上限・下限・上極限・下極限・極限関数]\label{def:pointwise-limits}
関数列 $f_n:X\to\eRR$ に対し，
...
\begin{proposition}[可算上限・可算下限の可測性]\label{prop:measurable-countable-sup-inf}
$(X,\calM)$ を可測空間とし，
...
\begin{proposition}[上極限・下極限・極限関数の可測性]\label{prop:measurable-limsup-liminf}
$(X,\calM)$ を可測空間とし，
$f_n\in M(X)$ $(n\in\NN)$ とする．
このとき
\[
...
\section{概収束} \label{sec:ae-convergence}
...
可測関数空間 $M(X)$ の元とは，\cref{thm:lebesgue-measurable-function-criterion}で見たように
...
\begin{enumerate}
    ...
    \item $M(X;\RR)$ は和・積・逆数・合成で閉じている（\cref{cor:measurable-algebra-operations,prop:measurable-reciprocal}）
    \item $M(X)$ は $\sup,\inf,\limsup,\liminf,\lim$ で閉じている（\cref{prop:measurable-countable-sup-inf,prop:measurable-limsup-liminf}）
