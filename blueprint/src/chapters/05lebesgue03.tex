...
\begin{lemma}[有限外測度集合のコンパクト近似判定法]\label{lem:finite-outer-compact-criterion}
$\lambda^*(A)<\infty$ とする．このとき次は同値である．
\begin{enumerate}
    \item $A$ はLebesgue可測である
...
Lebesgue外測度は外測度
（\cref{thm:lebesgue-outer-measure}）なので，$\lambda^*(A)\in[0,\infty]$ である．
また同じ外測度の基本性質より
\[
...
$\lambda(A)=\infty$ または $\lambda(B)=\infty$ のときは，
$A\subset A\cup B$ または $B\subset A\cup B$ と外測度の単調性
（\cref{thm:lebesgue-outer-measure}）より
\[
\lambda(A\cup B)=\infty
\]
となるのでよい．
以下では $\lambda(A),\lambda(B)<\infty$ とする．
...
\lambda_*(B)=\lambda(B)
\]
である．したがって内測度の定義より，任意の $\eps>0$ に対して
コンパクト集合 $K_A\subset A,\ K_B\subset B$ で
\[
\lambda^*(K_A)>\lambda(A)-\frac{\eps}{2},
\qquad
...
\]
となるものが取れる．
$A\cap B=\emptyset$ だから $K_A$ と $K_B$ は交わらない．
両方とも空でない場合，2つのコンパクト集合は正の距離だけ離れているので，
正距離集合に対する外測度の加法性
（\cref{lem:positive-distance-additive}）より
\[
\lambda^*(K_A\cup K_B)
...
\begin{theorem}[可算加法性]\label{thm:lebesgue-measure-countable-additivity}
...
\]
はLebesgue可測である．
外測度の可算劣加法性（\cref{thm:lebesgue-outer-measure}）より
\[
\lambda(A)
=
\lambda^*(A)
...
逆向きの不等式を示す．
\[
S_N:=\bigsqcup_{n=1}^{N}A_n
\]
とおく．有限加法性（\cref{lem:lebesgue-measure-finite-additivity}）を繰り返し用いると
\[
\lambda(S_N)=\sum_{n=1}^{N}\lambda(A_n)
\]
である．また $S_N\subset A$ なので，外測度の単調性
（\cref{thm:lebesgue-outer-measure}）より
\[
\sum_{n=1}^{N}\lambda(A_n)=\lambda(S_N)\le\lambda(A)
\]
...
\]
である．可算加法性（\cref{thm:lebesgue-measure-countable-additivity}）と
...
（\cref{lem:compact-splits-outer-measure}）より
\[
\lambda(A)=\lambda^*(A)=\lambda^*(K)+\lambda^*(A\setminus K)
\]
だから
\[
\lambda^*(K)>\lambda(A)-\eps
\]
である．コンパクト集合はLebesgue可測である
（\cref{thm:open-closed-lebesgue-measurable}）ので $\lambda^*(K)=\lambda(K)$ である．
したがって
\[
\sup\{\lambda(K)\mid K\subset A,\ K\text{ はコンパクト}\}
\ge
...
\]
である．逆向きの不等式は単調性
（\cref{thm:lebesgue-measure-basic}）から従う．
$\eps>0$ は任意なので，有限測度の場合の主張が従う．

次に $\lambda(A)=\infty$ とする．
\[
A_n:=A\cap Q_n\qquad(n\in\NN)
\]
とおく．Lebesgue可測集合の集合演算
（\cref{cor:lebesgue-measurable-set-operations}）と
開集合と閉集合の可測性
（\cref{thm:open-closed-lebesgue-measurable}）より，$A_n$ はLebesgue可測である．
また
\[
...
（\cref{thm:lebesgue-measure-continuity-from-below}）より
\[
\lambda(A)=\lim_{n\to\infty}\lambda(A_n)=\infty
\]
である．したがって任意の $M>0$ に対して，ある $n$ が存在して
\[
\lambda(A_n)>M+1
\]
となる．$A_n$ は有界なので $\lambda(A_n)<\infty$ であり，
すでに示した有限測度の場合を $A_n$ に適用すると，
コンパクト集合 $K\subset A_n\subset A$ で
\[
\lambda(K)>M
\]
となるものが取れる．よって
