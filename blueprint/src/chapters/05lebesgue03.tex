...
\begin{theorem}[有界閉区間内の $\sigma$-加法性]\label{thm:local-measurable-sigma-algebra}
...
\begin{proposition} \label{prop:lebesgue-global-local}
$A\subset\RR^d$ がLebesgue可測であること（$A \in \calL_d$）は，任意の $R>0$ に対して $A\cap Q_R\in\calL(Q_R)$ であることと同値である．    
\end{proposition}
証明は定義の言い換えから直ちに従う．

\begin{theorem}[Lebesgue可測集合族の $\sigma$-加法性]\label{thm:lebesgue-measurable-sigma-algebra}
...
\]
なので，\cref{prop:lebesgue-global-local} より $\RR^d \in \calL_d$ である．
%であり，$Q_R$ はコンパクト集合なので，コンパクト集合では内外が一致する（\cref{prop:compact-inner-outer-equal}）より内外一致する．
...
\begin{proposition}[非負性]\label{prop:lebesgue-measure-nonnegative}
...
Lebesgue外測度の非負性
（\cref{thm:lebesgue-outer-measure}）から従う．
% $\lambda^*(A)\in[0,\infty]$ である．
% また同じ外測度の基本性質より
% \[
...
\begin{lemma}[測度の極限表示]\label{lem:lebesgue-measure-limit}
...
\]
である．$m$ に関して上限を取ると，
\cref{lem:lebesgue-measure-limit} と単調列の上限の性質から
\[
\lambda(A\cup B)=\lambda(A)+\lambda(B)
\]
を得る．
...
\]
である．$m$ に関して上限を取り，
\cref{lem:lebesgue-measure-limit} を用いれば
\[
...
\cref{lem:lebesgue-measure-limit} と単調列の上限の性質より
\[
...
測度の極限表示（\cref{lem:lebesgue-measure-limit}）より
\[
...
\section{Carathéodory条件との比較}\label{sec:lebesgue-caratheodory-comparison}

\begin{definition}[Lebesgue外測度に関するCarathéodory可測性]\label{def:lebesgue-caratheodory-measurable}
...
\begin{lemma}[有限外測度集合のCarathéodory分割]\label{lem:lebesgue-measurable-caratheodory-finite}
...
開集合と閉集合の可測性
（\cref{thm:open-closed-lebesgue-measurable}）と
Lebesgue可測集合の集合演算
（\cref{cor:lebesgue-measurable-set-operations}）より，
...
\lambda^*(E)+\eps.
\]
$\eps>0$ は任意だから逆向きの不等式も従う．
\end{proof}

\begin{theorem}[Lebesgue可測ならCarathéodory可測]\label{thm:lebesgue-measurable-implies-caratheodory}
$A\in\calL_d$ ならば，$A$ は $\lambda^*$ に関してCarathéodory可測である．
\end{theorem}
\begin{proof}
任意の $E\subset\RR^d$ を取る．
$\lambda^*(E)<\infty$ の場合は
\cref{lem:lebesgue-measurable-caratheodory-finite} より従う．
...
\begin{theorem}[Carathéodory可測ならLebesgue可測]\label{thm:caratheodory-implies-lebesgue-measurable}
...
\]
である．区間の外測度は体積
（\cref{prop:interval-outer-measure-volume}）だから $\lambda^*(Q_R)=|Q_R|$ であり，
...
\]
である．従って区間による外測度の分解
（\cref{cor:interval-outer-measure-split}）により
\[
\lambda_*(A\cap Q_R)=\lambda^*(A\cap Q_R)
\]
...
\begin{corollary}[Lebesgue可測性とCarathéodory可測性の一致]\label{cor:lebesgue-caratheodory-equivalence}
集合 $A\subset\RR^d$ について，次は同値である．
\begin{enumerate}
    \item $A$ はLebesgue可測である
    \item $A$ は $\lambda^*$ に関してCarathéodory可測である
\end{enumerate}
\end{corollary}
\begin{proof}
\cref{thm:lebesgue-measurable-implies-caratheodory,thm:caratheodory-implies-lebesgue-measurable}
より従う．
\end{proof}
...
\begin{itemize}
    \item Lebesgue可測集合全体 $\calL_d$ は $\RR^d$ 上の $\sigma$-加法族である
    （\cref{thm:lebesgue-measurable-sigma-algebra}）．
    \item Lebesgue可測性は，Lebesgue外測度 $\lambda^*$ に関する
    Carathéodory可測性と同値である
