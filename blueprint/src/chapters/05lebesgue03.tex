...
\]
である．第3章の区間の外測度の計算
（\cref{prop:interval-outer-measure-volume}）より $\lambda^*(Q_R)=|Q_R|$ だから
\[
|Q_R|
=
\lambda^*(A\cap Q_R)+\lambda^*(Q_R\setminus A)
\]
である．区間による外測度の分解
（\cref{cor:interval-outer-measure-split}）を
$A\cap Q_R\subset Q_R$ に適用すると
（ここで $Q_R\setminus A=Q_R\setminus(A\cap Q_R)$ である）
\[
\lambda_*(A\cap Q_R)=\lambda^*(A\cap Q_R)
\]
