...
\begin{proposition} \label{prop:lebesgue-inner-basic}
...
\begin{proposition}[コンパクト集合では内外が一致する] \label{prop:compact-inner-outer-equal}
...
\begin{theorem} \label{thm:jordan-inner-lebesgue-inner}
...
\begin{theorem} \label{thm:inner-measure-via-open-set}
...
\begin{lemma}[コンパクト集合による外測度の分解]\label{lem:compact-splits-outer-measure}
...
\cref{lem:finite-open-compact-split} を $U$ と $C$ に適用すると
\[
\lambda^*(U)
=
...
\begin{corollary}[コンパクト集合を用いた内測度表示]\label{cor:inner-measure-via-compact-set}
...
開集合を用いた内測度表示（\cref{thm:inner-measure-via-open-set}）より
\[
\lambda_*(A)=\lambda^*(O)-\lambda^*(O\setminus A)
\]
である．
一方，\cref{lem:compact-splits-outer-measure} を $B=O\setminus A$ に適用すると
\[
\lambda^*(O\setminus A)
=
\lambda^*(C\setminus A)+\lambda^*(O\setminus C)
\]
である．また \cref{lem:finite-open-compact-split} より
\[
...
\begin{corollary}[区間を用いた内測度表示]\label{cor:inner-measure-via-interval}
...
\end{corollary}
\begin{proof}
$Q=\emptyset$ のときは自明である．
以下では $Q\neq\emptyset$ とする．
$C:=\overline Q$ とおくと，$C$ はコンパクト集合であり，$A\subset C$ である．
コンパクト集合を用いた内測度表示
（\cref{cor:inner-measure-via-compact-set}）より
\[
\lambda_*(A)
=
\lambda^*(C)-\lambda^*(C\setminus A)
...
\]
である．
また $C\setminus Q$ は有限個の座標超平面片の合併に含まれるので，
\[
\lambda^*(C\setminus Q)=0
\]
である．実際，各片は一方向だけ任意に薄い有界区間で覆える．
したがって
\[
C\setminus A=(Q\setminus A)\cup(C\setminus Q)
\]
より
\[
