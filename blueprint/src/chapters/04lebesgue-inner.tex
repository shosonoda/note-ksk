...
\begin{theorem}[区間を用いた内測度表示]\label{thm:inner-measure-via-interval}\label{cor:inner-measure-via-interval}
...
\cref{lem:compact-splits-outer-measure} を $B=C$ とコンパクト集合 $K$ に適用すると
\[
...
\begin{corollary}[区間による外測度の分解]\label{cor:interval-outer-measure-split}
有界区間 $Q\subset\RR^d$ と $A\subset Q$ に対して
\[
\lambda_*(A)=\lambda^*(A)
...
\end{corollary}
\begin{proof}
\cref{thm:inner-measure-via-interval} により
\[
