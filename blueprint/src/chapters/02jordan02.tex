...
\begin{lemma}[格子分割]
有限個の有界区間の合併は，基本集合として書き直せる．
特に，基本集合の表示に現れる区間の端点を全て集めて格子分割を取れば，
その各小区間は元の集合に含まれるか，さもなくば交わらないかのいずれかである．
\end{lemma}
\begin{proof}
各座標方向ごとに，もとの区間に現れる全ての端点を
\[
...
もとの各区間に対して「丸ごと含まれる」か「全く交わらない」かのどちらかである．
したがって，有限個の区間の合併は，そのうち必要な小区間だけを集めた基本集合として表せる．
\end{proof}
...
\begin{proof}
$E,F$ を共通の格子分割で書き直せば，
どちらも同じ小半開区間の有限個の合併として表せる．
すると $m_J$ は「含まれている小区間の体積を足し上げる」という形になるので，
非負性，単調性，有限加法性は直ちに従う．
また平行移動は各小区間の辺の長さを変えないから，体積も変わらない．
\end{proof}
...
特に $f = 1_A$ のとき，各小区間上での上限と下限は $0$ か $1$ しか取らないので，
\[
U(1_A,\calP)
= \sum_{R_j \cap A \neq \emptyset} |R_j|,
\qquad
...
格子分割を共通に細分すれば，$F$ と $G$ がいずれも分割の小区間の合併になる分割 $\calP$ が作れる．
そのとき
\[
L(1_A,\calP) \ge m_J(F),
\qquad
