...
\red{$\RR^d$の左半開区間の全体を$\calE_d$と書く．}
\end{definition}
...
以後，有限分割を扱う際には
\[
Q = \prod_{k=1}^d \red{(}a_k,b_k\red{]}
\]
という\red{左}半開区間を主に用いる．
...
\begin{lemma}[格子分割]
有限個の有界\red{左半}開区間$\{B_i\}_{i=1}^n \subset \RR^d$の合併 $A = \bigcup_{i=1}^n B_i$は，ある互いに素な半開区間$\{Q_j\}_{j=1}^m \subset \RR^d$からなる基本集合$\bigsqcup_{j=1}^m Q_j$として書き直せる．
\[
...
次が成り立つ．
\begin{enumerate}
    \item 有界基本集合 $E \subset \RR^d$ の境界 $\partial E$ はJordan零集合である．
    \item 任意の集合 $A_1,\dots,A_n \subset \RR^d$ に対して
    \[
...
\begin{enumerate}
    ...
    \item 右辺が $\infty$ のときは自明なので有限の場合を示せばよい．任意の $\eps>0$ に対し，各 $k$ に対して基本集合 $E_k$ が存在して
    \[
    A_k \subset E_k,
    \qquad
...
\begin{proposition}[分割によるJordan測度の近似]
...
\begin{remark}
この例が示す通り，Jordan可測集合は可算和で閉じていない．
