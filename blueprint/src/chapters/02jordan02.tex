...
\begin{lemma}[格子分割]
有限個の有界区間$\{B_i\}_{i=1}^n \subset \RR^d$の合併 $A = \bigcup_{i=1}^n B_i$は，ある互いに素な半開区間$\{Q_j\}_{j=1}^m \subset \RR^d$からなる基本集合$\bigsqcup_{j=1}^m Q_j$として書き直せる．
\[
A = \bigcup_{i=1}^n B_i = \bigsqcup_{j=1}^m Q_j.
\]
一般にこのような基本集合の取り方は一意ではないが，その体積$\bigsqcup_{j=1}^m |Q_j|$は基本集合の取り方に依らない．
\end{lemma}
\begin{proof}
...
\begin{enumerate}
    ...
    \]
    特に $A \cap B = \emptyset$ ならば
    \[
    m_J(A \sqcup B) = m_J(A) + m_J(B).
    \]
    \item （並行移動不変性）$m_J(A+c)=m_J(A)$
...
\begin{remark}
つまり，(1) 空集合を含み，(2)補集合演算と(3)有限合併について閉じた集合族を有限加法族という．
一方，全空間$X$を含む集合環を集合代数（algebra of sets）という．
また，有限交叉$A \cap B$と対称差$A \triangle B$について閉じた集合族を集合体（field of sets）という．これらの3つの定義は同値である．    
\end{remark}
...
\begin{example}
    $\RR^d$の基本集合の全体$\calA_d$は有限加法族であり，$m_J : \calA_d \to [0,\infty)$ は有限加法的測度である．
\end{example}
...
$m_J : \calA_X \to [0,\infty)$ は有限加法的測度である．
\end{example}
% \begin{proof}
...
\section{概念の整理}
\begin{itemize}
    \item （出発点）区間 $I = \prod_{i=1}^d (a_i,b_i]$ の体積 $|I| := \prod_{i=1}^d (b_i - a_i)$
    ...
    \item （拡張）Jordan可測集合 $A$ のJordan測度 $\mu_J(A) := \mu_J^*(A) = \mu_{J,*}(A)$
    \item 有限加法族の例：基本集合の全体 $\calA_d$，（$X$上の）Jordan可測集合の全体 $\calA_X$
