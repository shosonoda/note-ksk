...
ところが，\cref{cor:countable-lebesgue-null}より $\lambda(\QQ \cap [0,1])=0$ である．
すると，面積が $0$ の集合の上だけで値が $1$ になっている関数の積分値は $0$ であるべきだと考えるのが自然である．
...
&=
\sum_{i=1}^n a_i \sum_{j=1}^m \lambda(C_{ij}) \\
&=
\sum_{i=1}^n \sum_{j=1}^m a_i\,\lambda(C_{ij}) \\
&=
\sum_{i=1}^n \sum_{j=1}^m b_j\,\lambda(C_{ij}) \\
&=
\sum_{j=1}^m b_j \sum_{i=1}^n \lambda(C_{ij}) \\
&=
\sum_{j=1}^m b_j\,\lambda(B_j).
\end{align*}
したがって積分値は表示に依らない．
\end{proof}
...
\begin{enumerate}
    ...
    \[
    \int_E 1_A\,d\lambda = \lambda(A)
    \]
    である
    \item 定数関数 $c \ge 0$ に対して
    \[
    \int_E c\,d\lambda = c\,\lambda(E)
    \]
    である
    \item
    \[
    s=\sum_{k=1}^n a_k 1_{A_k}
    \]
    なら，積分は各値 $a_k$ にその値をとる領域の面積 $\lambda(A_k)$ を掛けて足したものである
\end{enumerate}
\end{example}
...
\begin{enumerate}
    \item
    \[
    \int_E (s+t)\,d\lambda = \int_E s\,d\lambda + \int_E t\,d\lambda
    \]
    \item
    \[
    \int_E cs\,d\lambda = c \int_E s\,d\lambda
    \]
    \item $s \le t$ なら
    \[
    \int_E s\,d\lambda \le \int_E t\,d\lambda
    \]
\end{enumerate}
\end{proposition}
\begin{proof}
...
\int_E (s+t)\,d\lambda
&=
\sum_{i,j}(a_i+b_j)\lambda(C_{ij}) \\
&=
\sum_{i,j}a_i \lambda(C_{ij})+\sum_{i,j}b_j \lambda(C_{ij}) \\
&=
\sum_i a_i \lambda(A_i)+\sum_j b_j \lambda(B_j) \\
&=
\int_E s\,d\lambda+\int_E t\,d\lambda.
\end{align*}
これで (1) が従う．
...
\int_E t\,d\lambda
=
\int_E s\,d\lambda+\int_E (t-s)\,d\lambda
\ge
\int_E s\,d\lambda
\]
である．
...
\begin{definition}[非負可測関数のLebesgue積分]\label{def:nonnegative-lebesgue-integral}
...
定義より
\[
\int_E s_n\,d\lambda \le \int_E f\,d\lambda.
\]
また $s_n \le s_{n+1}$ だから，単関数の単調性より
\[
\int_E s_n\,d\lambda \le \int_E s_{n+1}\,d\lambda.
\]
したがって列
\[
\left\{\int_E s_n\,d\lambda\right\}_{n=1}^{\infty}
\]
は単調増加であり，極限
\[
L:=\lim_{n\to\infty}\int_E s_n\,d\lambda
\]
が存在して
\[
...
したがって
\[
\int_E u_n\,d\lambda \le \int_E s_n\,d\lambda \le L.
\]

一方，測度の下からの連続性（\cref{thm:lebesgue-measure-continuity-from-below}）より
\[
\lambda(A_{j,n}) \to \lambda(A_j)
\]
であるから，
\[
\int_E u_n\,d\lambda
=
\sum_{j=1}^m \left(a_j-\frac1n\right)_+ \lambda(A_{j,n})
\to
\sum_{j=1}^m a_j \lambda(A_j)
=
\int_E t\,d\lambda.
\]
よって
\[
\int_E t\,d\lambda \le L.
\]
...
\int_E (f+g)\,d\lambda
&=
\lim_{n\to\infty}\int_E (s_n+t_n)\,d\lambda \\
&=
\lim_{n\to\infty}\left(\int_E s_n\,d\lambda+\int_E t_n\,d\lambda\right) \\
&=
...
\int_E cf\,d\lambda
=
\lim_{n\to\infty}\int_E cs_n\,d\lambda
=
\lim_{n\to\infty} c\int_E s_n\,d\lambda
=
c\int_E f\,d\lambda
\]
を得る．
...
\int_E g\,d\lambda
=
\int_E f\,d\lambda+\int_E (g-f)\,d\lambda
\ge
\int_E f\,d\lambda
\]
であり，(3) が従う．
...
\int_{A \sqcup B} f\,d\lambda
&=
\int_E f1_{A \sqcup B}\,d\lambda \\
&=
\int_E f(1_A+1_B)\,d\lambda \\
&=
\int_E f1_A\,d\lambda+\int_E f1_B\,d\lambda \\
&=
\int_A f\,d\lambda+\int_B f\,d\lambda.
\end{align*}
\end{proof}
...
$N \subset E$ が可測で $\lambda(N)=0$ ならば，
...
\[
\int_N f\,d\lambda=\int_N g\,d\lambda=0.
\]
...
\int_E f\,d\lambda
=
\int_{E\setminus N} f\,d\lambda+\int_N f\,d\lambda
=
\int_{E\setminus N} g\,d\lambda+\int_N g\,d\lambda
=
\int_E g\,d\lambda.
\]
\end{proof}
...
\[
\int_E f\,d\lambda=\int_E 0\,d\lambda=0
\]
である．
...
\frac1n \lambda(A_n)
=
\int_E \frac1n 1_{A_n}\,d\lambda
\le
\int_E f\,d\lambda
=
0.
\]
よって $\lambda(A_n)=0$ である．
一方，
\[
\{x \in E \mid f(x)>0\}
=
...
\int_E f\,d\lambda
:=
\int_E f^+\,d\lambda-\int_E f^-\,d\lambda
\]
で定める．
\end{definition}
...
$u,v,u',v'$ がすべて非負可測で
\[
\int_E u\,d\lambda,\ \int_E v\,d\lambda,\ \int_E u'\,d\lambda,\ \int_E v'\,d\lambda<\infty
\]
とする．
このとき
\[
...
したがって右辺が有限であることと
\[
\int_E f^+\,d\lambda<\infty,\qquad \int_E f^-\,d\lambda<\infty
\]
が成り立つことは同値である．
...
\int_E |f|\,d\lambda
=
\int_E |f|1_{E\setminus N}\,d\lambda.
\]
...
だから，単調性より
\[
n\,\lambda(N)=\int_E n1_N\,d\lambda \le \int_E |f|\,d\lambda<\infty.
\]
これがすべての $n$ で成り立つためには
\[
\lambda(N)=0
...
したがって
\[
\int_N f^+\,d\lambda=\int_N f^-\,d\lambda=0
\]
である．
よって $f$ は $N$ 上で可積分で，
\[
\int_N f\,d\lambda
=
\int_N f^+\,d\lambda-\int_N f^-\,d\lambda
=
0
\]
となる．
...
\[
\int_E h\,d\lambda-\int_E s\,d\lambda<\frac{\eps}{2}
\]
となる．
いま
\[
...
\]
とおく．
可測集合 $A \subset E$ が $\lambda(A)<\delta$ を満たすとする．
すると集合に関する加法性と $s\le h$ から
\begin{align*}
\int_E h\,d\lambda
&=
\int_A h\,d\lambda+\int_{E\setminus A}h\,d\lambda \\
&\ge
\int_A h\,d\lambda+\int_{E\setminus A}s\,d\lambda.
\end{align*}
よって
\begin{align*}
\int_A h\,d\lambda
&\le
\int_E h\,d\lambda-\int_{E\setminus A}s\,d\lambda \\
&=
\left(\int_E h\,d\lambda-\int_E s\,d\lambda\right)+\int_A s\,d\lambda \\
&<
\frac{\eps}{2}+M\,\lambda(A) \\
&<
...
\int_{[0,1]} f\,d\lambda
=
\int_{[0,1]} 1_{\QQ \cap [0,1]}\,d\lambda
=
\lambda(\QQ \cap [0,1])
=
0.
...
さらに
\[
\int_I \phi_n\,d\lambda = L(f,\calP_n),
\qquad
\int_I \psi_n\,d\lambda = U(f,\calP_n)
\]
である．
...
$I$ は有界閉区間なので
\[
\int_I M1_I\,d\lambda = M\,\lambda(I)=M|I|<\infty
\]
である．
