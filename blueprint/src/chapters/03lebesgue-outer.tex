...
次の定理は非自明であり，
Lebesgue外測度が長方体の体積と整合することを保証する．

\begin{theorem}[長方体の外測度]\label{thm:outer-measure-rectangle}
半開長方体 $Q = \prod_{k=1}^d [a_k, b_k)$ に対して
\[
...
$\eps > 0$ を取る．$Q$ をわずかに縮めた閉長方体
\[
\overline{Q}_\delta
= \prod_{k=1}^d [a_k + \delta,\ b_k - \delta]
\qquad (\delta > 0 \text{ は十分小さく取る})
\]
を考え，各 $Q_j$ をわずかに膨らませた開長方体 $Q_j^\circ$ を
\[
...
本質的に使われている．
半開長方体を閉長方体で内側から近似し（コンパクト性を確保），
被覆の長方体を開長方体で外側から近似して（開被覆を構成），
有限部分被覆に帰着させる．
...
$E = \bigsqcup_{l=1}^n P_l$（$P_l$ は半開長方体）と書く．
各 $R_k$ と $P_l$ の交差 $R_k \cap P_l$ は半開長方体であり，
...
\begin{proof}
$\eps > 0$ を取る．
各 $a_j$ を一辺の長さ $(\eps/n)^{1/d}$ の半開長方体 $Q_j$ で覆えば
...
\begin{proof}
$\eps > 0$ を固定する．
各 $a_n$ を一辺の長さ $(\eps/2^n)^{1/d}$ の半開長方体 $Q_n$ で覆えば
$|Q_n| = \eps/2^n$ であり
\[
