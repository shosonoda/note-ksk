...
\begin{theorem}[可算集合は零集合] \label{thm:countable-outer-measure-zero}
...
%\end{enumerate}
\end{example}
\begin{proposition}\label{prop:interval-outer-measure-volume}
各 $a_i<b_i$ を実数とする．
開区間$R=\prod_{i=1}^d (a_i,b_i)$,
左半開区間$R=\prod_{i=1}^d (a_i,b_i]$,
右半開区間$R=\prod_{i=1}^d [a_i,b_i)$,
閉区間$R=\prod_{i=1}^d [a_i,b_i]$
...
\begin{theorem}[Lebesgue外測度は外測度] \label{thm:lebesgue-outer-measure}
...
\begin{proposition}[平行移動不変性] \label{prop:lebesgue-outer-translation-invariant}
...
\begin{theorem}[開集合による外正則性] \label{thm:lebesgue-outer-regularity-open}
任意の $A \subset \RR^d$ に対して
\[
\lambda^*(A)
=
