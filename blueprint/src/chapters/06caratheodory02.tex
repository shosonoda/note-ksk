...
$X$ 上の $\sigma$-加法族であるとは，
$X\in\calS$，$A\in\calS$ ならば $X\setminus A\in\calS$，
また $\{A_n\}_{n=1}^{\infty}\subset\calS$ ならば
$\bigcup_{n=1}^{\infty}A_n\in\calS$ であることをいう．
\end{definition}
...
ある $X_n\in\calS$ が存在して
$X=\bigcup_{n=1}^{\infty}X_n$ かつ $\mu(X_n)<\infty$ を満たすことをいう．
...
$\mu^*(\emptyset)=0$，$A\subset B$ ならば $\mu^*(A)\le\mu^*(B)$，
また任意の集合列 $\{A_n\}_{n=1}^{\infty}\subset\calP(X)$ に対して
...
一般の測度の完備化とは，零集合の部分集合をすべて可測集合として加えた測度であり，
たとえば
...
$\emptyset\in\calA$，$A,B\in\calA$ ならば $A\cup B\in\calA$，
また $A,B\in\calA$ ならば $A\setminus B\in\calA$ であることをいう．
\end{definition}

\begin{definition}[半加法族（半環）]\label{def:semiring}
集合 $X$ の部分集合族 $\calE\subset\calP(X)$ が半加法族，または半環，であるとは，
$\emptyset\in\calE$，$A,B\in\calE$ ならば $A\cap B\in\calE$，
また $A,B\in\calE$ ならば $A\setminus B$ が有限個の互いに素な $\calE$ の元の合併
として書けることをいう．
ここで有限個は $0$ 個でもよい．
\end{definition}
...
$\mu_0(\emptyset)=0$ であり，互いに素な列 $\{E_n\}\subset\calE$ が
$E=\bigsqcup_{n=1}^{\infty}E_n\in\calE$ を満たすとき
$\mu_0(E)=\sum_{n=1}^{\infty}\mu_0(E_n)$ となることをいう．
...
$\mu_0(\emptyset)=0$，$\mu_0$ が有限加法的であり，互いに素な列
$\{A_n\}\subset\calA$ について $\bigcup_n A_n\in\calA$ であるとき
$\mu_0(\bigcup_n A_n)=\sum_n\mu_0(A_n)$ が成り立つことをいう．
\end{definition}
...
$\overline\mu_0(A):=\sum_{j=1}^{N}\mu_0(E_j)$ と定めると，
$\overline\mu_0$ は表示の取り方によらず定まり，
$\calA(\calE)$ 上の前測度になる．
\end{theorem}
\begin{proof}
...
\cref{def:outer-measure-generated-by-premeasure}で定めた $\mu^*$ は $X$ 上の外測度である．
さらに，任意の $A\in\calA$ に対して $\mu^*(A)=\mu_0(A)$ であり，
$A$ は $\mu^*$-可測である．
\end{theorem}
\begin{proof}
...
このとき，$\sigma(\calE)$ 上の測度 $\mu$ で
すべての $E\in\calE$ に対して $\mu(E)=\mu_0(E)$ を満たすものが存在する．
さらに，$\overline\mu_0$ が $\calA(\calE)$ 上で $\sigma$-有限ならば，
この拡張は一意的である．
\end{theorem}
\begin{proof}
...
任意のLebesgue可測集合 $A\subset\RR^d$ は，
あるBorel集合 $B$ とあるLebesgue零集合 $N$ を用いて $A=B\triangle N$ と書ける．
したがってLebesgue測度は，Borel測度の完備化である．
\end{theorem}
\begin{proof}
