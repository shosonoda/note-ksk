...
Lebesgue可測集合全体を
\[
\calL_d:=\{A\subset\RR^d\mid \lambda_*(A)=\lambda^*(A)\}
\]
と書く．
$A\in\calL_d$ のとき，共通値
\[
...
\begin{lemma}[有界集合の区間判定]\label{lem:bounded-measurable-interval-criterion}
...
\end{lemma}
\begin{proof}
\cref{cor:inner-measure-via-interval} より
\[
...
\lambda^*(A)+\lambda^*(Q\setminus A)=|Q|
\]
だから，\cref{lem:bounded-measurable-interval-criterion} より $A$ はLebesgue可測である．
さらに
\[
\lambda(A)=m_J(A)
\]
...
\end{lemma}
\begin{proof}
$\calL_d$ は $\sigma$-加法族なので，各 $B_n$ はLebesgue可測である．
定義から互いに素性と表示は明らかである．
\end{proof}
...
\begin{enumerate}
    ...
    \]
    \item 完全劣加法性：$\{A_n\}_{n=1}^{\infty}\subset\calL_d$ ならば
    \[
...
$\calL_d$ は $\sigma$-加法族だから，$G$ はLebesgue可測である．
\end{proof}
