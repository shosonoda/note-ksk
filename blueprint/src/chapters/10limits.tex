...
$f_n\in M(X;\RR)$ $(n\in\NN)$，$f\in M(X;\RR)$ とし，$g\in M^+(X)\cap\calL^1(X)$ とする．
...
\begin{corollary}[$L^1$ 収束版の優収束定理]\label{cor:dominated-convergence-l1}
\cref{thm:dominated-convergence}と同じ仮定のもとで
\[
...
\end{corollary}
\begin{proof}
\cref{thm:dominated-convergence}の証明中で示した通りである．
\end{proof}

\begin{corollary}[有界収束定理]\label{cor:bounded-convergence}
$\mu(X)<\infty$ とし，
$f_n\in M(X;\RR)$ $(n\in\NN)$，$f\in M(X;\RR)$ とする．
もし $f_n(x)\to f(x)$ a.e.\ on $X$ かつある定数 $M>0$ が存在して
$|f_n(x)|\le M$ a.e.\ on $X$ がすべての $n$ について成り立つならば，
$f\in\calL^1(X)$ で $\int_X f_n\,d\mu\to \int_X f\,d\mu$ が成り立つ．
\end{corollary}
\begin{proof}
...
$f_n\in\calL^1(X)$ $(n\in\NN)$ とする．
さらに，ある $g\in M^+(X)\cap\calL^1(X)$ が存在して
$\sum_{n=1}^{\infty}|f_n(x)|\le g(x)$ a.e.\ on $X$ を満たすとする．
このとき級数 $\sum_{n=1}^{\infty}f_n(x)$ は a.e.\ で絶対収束し，
その和を $f$ と書けば $f\in\calL^1(X)$ で，
...
\section{積分記号下の連続性と微分}

\begin{theorem}[積分記号下の連続性]\label{thm:continuity-under-integral-sign}
$I\subset\RR$ を開区間とし，$t_0\in I$ とする．
関数 $F:I\times X\to\RR$ が次を満たすとする．
\begin{enumerate}
    \item 各 $t\in I$ に対して，$F(t,\cdot)\in M(X;\RR)$ である
    \item ほとんどすべての $x\in X$ に対して，$t\mapsto F(t,x)$ は $t_0$ で連続である
    \item ある $g\in M^+(X)\cap\calL^1(X)$ が存在して，$|F(t,x)|\le g(x)$ がすべての $t\in I$ と a.e.\ の $x\in X$ で成り立つ
\end{enumerate}
このとき
\[
...
したがって\cref{thm:dominated-convergence}より
\[
\int_X F(t_n,x)\,d\mu(x)\to \int_X F(t_0,x)\,d\mu(x)
\]
である．
これは $G$ が $t_0$ で連続であることを意味する．
\end{proof}
...
\begin{enumerate}
    ...
    \item 各 $t \in I$ に対して，$F(t,\cdot)$ と $\partial_tF(t,\cdot)$ は $M(X;\RR)$ に属する
    \item ある $t_0 \in I$ について $F(t_0,\cdot)\in\calL^1(X)$ である
    \item ある $g\in M^+(X)\cap\calL^1(X)$ が存在して，$|\partial_tF(t,x)|\le g(x)$ がすべての $t \in I$ と a.e.\ の $x \in X$ で成り立つ
\end{enumerate}
...
\begin{enumerate}
    ...
    \item $|f_n|\le g\in\calL^1(X)$ なら $\int f_n \to \int f$
