...
区間を用いた内測度表示（\cref{cor:inner-measure-via-interval}）を
...
コンパクト集合 $D$ と集合 $A,\,C\setminus A,\,C$ にそれぞれ適用すると，
\[
\lambda^*(A)=\lambda^*(E)+\lambda^*(A\setminus D),
\]
\[
\lambda^*(C\setminus A)
=
\lambda^*(D\setminus E)
+
\lambda^*((C\setminus A)\setminus D),
\]
\[
\lambda^*(C)=\lambda^*(D)+\lambda^*(C\setminus D)
\]
である．また
\[
C\setminus D
=
(A\setminus D)\cup((C\setminus A)\setminus D)
\]
だから，外測度の劣加法性より
\[
\lambda^*(C\setminus D)
...
\]
である．
すべて $C$ の部分集合なので外測度は有限であり，上の等式を比較すると
\[
\lambda^*(E)+\lambda^*(D\setminus E)
\le
\lambda^*(D)
\]
を得る．逆向きの不等式は外測度の劣加法性から従うので
\[
\lambda^*(D)=\lambda^*(E)+\lambda^*(D\setminus E)
\]
である．
最後に，区間を用いた内測度表示を $E\subset D$ に適用すれば
\[
\lambda_*(E)
=
\lambda^*(D)-\lambda^*(D\setminus E)
=
\lambda^*(E)
\]
を得る．
...
区間を用いた内測度表示（\cref{cor:inner-measure-via-interval}）より，
$A$ がLebesgue可測であることは
\[
\lambda^*(A)=\lambda_*(A)=|Q|-\lambda^*(Q\setminus A)
\]
と同値である．
\end{proof}
